#!/usr/bin/env python3
"""
Quantitative Evaluation of Grad-CAM against Ground-Truth Bounding Boxes / Masks.
...
Usage:
  python eval_gradcam_metrics.py --dataset bdd100k --data-dir ./data/bdd100k --checkpoint /path/to/last.pth
  python eval_gradcam_metrics.py --dataset voc --data-dir ./data --checkpoint /path/to/last.pth --max-samples 500
"""
...
from voc_dataset import build_voc_datasets, detection_collate, VOC_CLASSES
...
def main():
    ...
    parser.add_argument(
        "--dataset", type=str, default="bdd100k", choices=["voc", "coco", "bdd100k"]
    )
    ...
    print(f"  Arch:         {args.arch} / {args.model}")
    ...
    dataloader = DataLoader(val_dataset, batch_size=1, shuffle=False, collate_fn=collate_fn)

    # Build model
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    if args.arch == "maskrcnn":
        model = FastViTMaskRCNN(num_classes=num_classes + 1)
    else:
