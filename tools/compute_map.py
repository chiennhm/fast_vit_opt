...
(e.g., by train_bdd100k.py with --save-json), computes detection metrics
...
Usage:
    ...
    python -m tools.compute_map --json-dir ./output/bdd100k/fastvit_sa12_RUN/json_results

    # Explicit file paths and custom save directory
    python -m tools.compute_map \
...
def main():
    ...
    if getattr(args, "eval_weather", True):
    ...
    if getattr(args, "visualize_failed_cases", False):
        ...
        from tools.visualize_failures import main as viz_main
        failed_save_dir = os.path.join(save_dir, "failed_cases_per_class")
        sys.argv = [
            "tools.visualize_failures",
            "--pred-json", pred_json,
            "--gt-json", gt_json,
