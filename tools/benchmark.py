...
#
# Usage:
#   python -m tools.benchmark --model fastvit_sa12 --checkpoint best.pth
#   python -m tools.benchmark --model all --img-size 512 --batch-size 1 4
#
...
# ============================================================================
def main():
    parser = argparse.ArgumentParser(description="FastViT BDD100K detector benchmark")
    parser.add_argument(
        "--model",
        type=str,
        nargs="+",
    ...
    for variant in variants:
        for img_size in args.img_size:
            for batch_size in args.batch_size:
                tag = f"{variant} | detection | {img_size}px | bs={batch_size}"
                print(f"\n>>> {tag}")
