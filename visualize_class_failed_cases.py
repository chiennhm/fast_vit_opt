...
for each class in a dataset (e.g., BDD100K, PASCAL VOC, MS-COCO).
...
def main():
    ...
    parser.add_argument(
        "--dataset", type=str, default="bdd100k", choices=["bdd100k", "voc", "coco"],
        help="Dataset name"
    )
    ...
    # Class names & Default img_dir
    if args.dataset == "bdd100k":
        class_names = BDD100K_CLASSES
        if not args.img_dir:
            args.img_dir = "./data/bdd100k/val/images"
        if not args.coco_ann and os.path.isfile("./data/bdd100k/val/annotations/coco_instances_val.json"):
            args.coco_ann = "./data/bdd100k/val/annotations/coco_instances_val.json"
    elif args.dataset == "voc":
        class_names = VOC_CLASSES
        if not args.img_dir:
            args.img_dir = "./data/VOCdevkit/VOC2007/JPEGImages"
    else:
        class_names = COCO_CLASSES
        if not args.img_dir:
            args.img_dir = "./data/coco/val2017"

    logger.info(f"Loading predictions & GT for dataset '{args.dataset}'...")
